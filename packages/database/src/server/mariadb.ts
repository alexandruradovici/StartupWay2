...
export class MariaDBServer {
	...
	private async createDB():Promise<void> {
		try {
			...
			const r:any[] = await this.conn.query(queryOptions, values);
			await this.pool.end();
			if(r[0] === undefined || r[0].length < 1) {
				...
				// workshops
				await this.conn.query("CREATE TABLE `workshops` (`workshopId` int(11) NOT NULL AUTO_INCREMENT,`workshopName` varchar(120) NOT NULL,PRIMARY KEY (`workshopId`),UNIQUE KEY `workshopName` (`workshopName`)) ENGINE=InnoDB AUTO_INCREMENT=3 ");
				// workshopInstances
