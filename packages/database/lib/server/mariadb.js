...
var MariaDBServer = /** @class */ (function () {
    ...
    MariaDBServer.prototype.createPoolConn = function () {
        try {
            ...
            // 	if((conns as any)[(conn.threadId as any)] === undefined) {
            // 		(conns as any) [(conn.threadId as any)] = 1;
            // 	} else {
            // 		(conns as any) [(conn.threadId as any)]++;
            // 	}
            // });
    ...
    MariaDBServer.prototype.createDB = function () {
        return __awaiter(this, void 0, void 0, function () {
            var auxPool, queryOptions, r, tablePool, respPool, error_1;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                        ...
                        return [4 /*yield*/, auxPool.query(queryOptions, { db: process.env.DB_NAME })];
                    case 1:
                        r = _a.sent();
                        if (!(r[0] === undefined || r[0].length < 1)) return [3 /*break*/, 18];
                        queryOptions = {
                            namedPlaceholders: true,
                            sql: "CREATE DATABASE " + tables_1.DB_NAME + " CHARACTER SET " + tables_1.DB_CHARSET + " COLLATE " + tables_1.DB_COLLATE
                        };
                        return [4 /*yield*/, auxPool.query(queryOptions)];
                    case 2:
                        _a.sent();
                        ...
                        // users
                        return [4 /*yield*/, tablePool.query("CREATE TABLE `users` (`userId` varchar(100) NOT NULL, `firstName` varchar(50) NOT NULL,`lastName` varchar(50) NOT NULL,`username` varchar(120) NOT NULL,`password` varchar(90) NOT NULL,`email` varchar(100) NOT NULL,`phone` varchar(15) NOT NULL,`birthDate` datetime NOT NULL,`avatarUu` varchar(100) NOT NULL,`socialMedia` text NOT NULL,`userDetails` text NOT NULL,`role` text NOT NULL,`lastLogin` datetime NOT NULL,PRIMARY KEY (`userId`),UNIQUE KEY `username` (`username`),UNIQUE KEY `username_password` (`username`,`password`)) ENGINE=InnoDB")];
                    case 3:
                        // TODO each package register it's table
                        // users
                        _a.sent();
                        // products
                        return [4 /*yield*/, tablePool.query("CREATE TABLE `products` (`productId` varchar(100) NOT NULL, `startupName` varchar(30) NOT NULL,`businessTrack` varchar(60) NOT NULL,`teamType` varchar(30) NOT NULL,`workshopDay` varchar(30) NOT NULL,`mentorId` varchar(100) NOT NULL,`descriptionRO` varchar(600) NOT NULL,`descriptionEN` varchar(600) NOT NULL,`pendingDescriptionRO` varchar(600) NOT NULL,`pendingDescriptionEN` varchar(600) NOT NULL,`userId` varchar(100) DEFAULT NULL,`productDetails` text NOT NULL,`updatedAt` timestamp NOT NULL,`lastMentorUpdate` timestamp NOT NULL,PRIMARY KEY (`productId`),UNIQUE KEY `startupName` (`startupName`),KEY `FK_99d90c2a483d79f3b627fb1d5e9` (`userId`),CONSTRAINT `FK_99d90c2a483d79f3b627fb1d5e9` FOREIGN KEY (`userId`) REFERENCES `users` (`userId`) ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE=InnoDB")];
                    case 4:
                        // products
                        _a.sent();
                        // teams
                        return [4 /*yield*/, tablePool.query("CREATE TABLE `teams` (`teamId` varchar(100) NOT NULL, `productId` varchar(100) NOT NULL,`year` int(11) NOT NULL,`location` varchar(100) NOT NULL,`teamName` varchar(120) NOT NULL,`teamDetails` text NOT NULL,PRIMARY KEY (`teamId`),UNIQUE KEY `teamName` (`teamName`),UNIQUE KEY `REL_004cf2c4fc89735735b1e77a6d` (`productId`),CONSTRAINT `FK_004cf2c4fc89735735b1e77a6db` FOREIGN KEY (`productId`) REFERENCES `products` (`productId`) ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE=InnoDB")];
                    case 5:
                        // teams
                        _a.sent();
                        // sessions
                        return [4 /*yield*/, tablePool.query("CREATE TABLE `sessions` (`userId` varchar(100) NOT NULL,`sessionId` varchar(100) NOT NULL, `token` varchar(150) NOT NULL,`createdAt` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6),PRIMARY KEY (`sessionId`),KEY `FK_57de40bc620f456c7311aa3a1e6` (`userId`),CONSTRAINT `FK_57de40bc620f456c7311aa3a1e6` FOREIGN KEY (`userId`) REFERENCES `users` (`userId`) ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE=InnoDB")];
                    case 6:
                        // sessions
                        _a.sent();
                        // userTeams
                        return [4 /*yield*/, tablePool.query("CREATE TABLE `userTeams` (`userProductId` varchar(100) NOT NULL, `userId` varchar(100) NOT NULL,`teamId` varchar(100) NOT NULL,`role` varchar(10) NOT NULL,PRIMARY KEY (`userProductId`),KEY `FK_83be579ba81c5f68b6792c4fe4f` (`userId`),KEY `FK_be4217fd0c329929e298f276f3e` (`teamId`),CONSTRAINT `FK_83be579ba81c5f68b6792c4fe4f` FOREIGN KEY (`userId`) REFERENCES `users` (`userId`) ON DELETE NO ACTION ON UPDATE NO ACTION,CONSTRAINT `FK_be4217fd0c329929e298f276f3e` FOREIGN KEY (`teamId`) REFERENCES `teams` (`teamId`) ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE=InnoDB")];
                    case 7:
                        // userTeams
                        _a.sent();
                        // userActivities
                        return [4 /*yield*/, tablePool.query("CREATE TABLE `userActivities` (`activityId` varchar(100) NOT NULL, `userId` varchar(100) NOT NULL,`teamId` varchar(100) NOT NULL,`date` datetime NOT NULL,`description` varchar(1000) NOT NULL,`noOfHours` int(11) NOT NULL,PRIMARY KEY (`activityId`),KEY `FK_274a34b4f5e47d9ca5ddeccc922` (`userId`),KEY `FK_ba771a2ce2af5e6cfa9e552b6db` (`teamId`),CONSTRAINT `FK_274a34b4f5e47d9ca5ddeccc922` FOREIGN KEY (`userId`) REFERENCES `users` (`userId`) ON DELETE NO ACTION ON UPDATE NO ACTION,CONSTRAINT `FK_ba771a2ce2af5e6cfa9e552b6db` FOREIGN KEY (`teamId`) REFERENCES `teams` (`teamId`) ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE=InnoDB")];
                    case 8:
                        // userActivities
                        _a.sent();
                        // bModelCanvas
                        return [4 /*yield*/, tablePool.query("CREATE TABLE `bModelCanvas` (`modelId` varchar(100) NOT NULL, `productId` varchar(100) NOT NULL,`date` datetime NOT NULL,`fields` text NOT NULL,PRIMARY KEY (`modelId`),KEY `FK_9b5ad0d5379a1f0e9165fc40408` (`productId`),CONSTRAINT `FK_9b5ad0d5379a1f0e9165fc40408` FOREIGN KEY (`productId`) REFERENCES `products` (`productId`) ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE=InnoDB")];
                    case 9:
                        // bModelCanvas
                        _a.sent();
                        // workshops
                        return [4 /*yield*/, tablePool.query("CREATE TABLE `workshops` (`workshopId` varchar(100) NOT NULL, `workshopName` varchar(120) NOT NULL,PRIMARY KEY (`workshopId`),UNIQUE KEY `workshopName` (`workshopName`)) ENGINE=InnoDB ")];
                    case 10:
                        // workshops
                        _a.sent();
                        // workshopInstances
                        return [4 /*yield*/, tablePool.query("CREATE TABLE `workshopInstances` (`workshopInstanceId` varchar(100) NOT NULL, `workshopId` varchar(100) NOT NULL,`teamId` varchar(100) NOT NULL,`trainerName` varchar(100) NOT NULL,`workshopDate` datetime NOT NULL,`workshopDetails` text NOT NULL,PRIMARY KEY (`workshopInstanceId`),UNIQUE KEY `workshopInstanceId` (`workshopInstanceId`),KEY `FK_2e20c0c8114652d2f442cedc6e1` (`workshopId`),KEY `FK_49ca002b40c714b757ae74c323a` (`teamId`),CONSTRAINT `FK_2e20c0c8114652d2f442cedc6e1` FOREIGN KEY (`workshopId`) REFERENCES `workshops` (`workshopId`) ON DELETE NO ACTION ON UPDATE NO ACTION,CONSTRAINT `FK_49ca002b40c714b757ae74c323a` FOREIGN KEY (`teamId`) REFERENCES `teams` (`teamId`) ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE=InnoDB")];
                    case 11:
                        // workshopInstances
                        _a.sent();
                        // workshopAttendances
                        return [4 /*yield*/, tablePool.query("CREATE TABLE `workshopAttendances` (`attendanceId` varchar(100) NOT NULL, `workshopInstanceId` varchar(100) NOT NULL,`attendanceDate` datetime NOT NULL,`userId` varchar(100) NOT NULL,PRIMARY KEY (`attendanceId`),UNIQUE KEY `attendanceId` (`attendanceId`),KEY `FK_eb9bcf0843c6ac3782a9f2fa4cf` (`workshopInstanceId`),KEY `FK_772b3a178a351d44ea62d230f53` (`userId`),CONSTRAINT `FK_772b3a178a351d44ea62d230f53` FOREIGN KEY (`userId`) REFERENCES `users` (`userId`) ON DELETE NO ACTION ON UPDATE NO ACTION,CONSTRAINT `FK_eb9bcf0843c6ac3782a9f2fa4cf` FOREIGN KEY (`workshopInstanceId`) REFERENCES `workshopInstances` (`workshopInstanceId`) ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE=InnoDB")];
                    case 12:
                        // workshopAttendances
                        _a.sent();
                        // uploadDownload
                        return [4 /*yield*/, tablePool.query("CREATE TABLE `uploadDownload` (`uuid` varchar(100) NOT NULL,`productId` varchar(100) NOT NULL,`fileType` varchar(255) NOT NULL,`extension` varchar(255) NOT NULL,`uploadTime` datetime NOT NULL,PRIMARY KEY (`uuid`),KEY `FK_0af268615f35d00781c400254ae` (`productId`),CONSTRAINT `FK_0af268615f35d00781c400254ae` FOREIGN KEY (`productId`) REFERENCES `products` (`productId`) ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE=InnoDB")];
                    case 13:
                        // uploadDownload
                        _a.sent();
                        // recoveries
                        return [4 /*yield*/, tablePool.query("CREATE TABLE `recoveries` (`recoveryId` varchar(100) NOT NULL, `userId` varchar(100) NOT NULL,`email` varchar(100) NOT NULL,`recoveryLink` varchar(100) NOT NULL,PRIMARY KEY (`recoveryId`),UNIQUE KEY `token` (`recoveryLink`),KEY `FK_7902dfcee5a16cb1377e04d3d7a` (`userId`),CONSTRAINT `FK_7902dfcee5a16cb1377e04d3d7a` FOREIGN KEY (`userId`) REFERENCES `users` (`userId`) ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE=InnoDB")];
                    case 14:
                        // recoveries
                        _a.sent();
                        // feeds
                        return [4 /*yield*/, tablePool.query("CREATE TABLE `feeds` (`feedId` varchar(100) NOT NULL,  `teamId` varchar(100) NOT NULL, `feedType` varchar(30) NOT NULL, `text` text NOT NULL, `date` datetime NOT NULL, PRIMARY KEY (`feedId`), KEY `FK_87caf98485e27800f1e171ccf6c` (`teamId`), CONSTRAINT `FK_87caf98485e27800f1e171ccf6c` FOREIGN KEY (`teamId`) REFERENCES `teams` (`teamId`) ON DELETE NO ACTION ON UPDATE NO ACTION) ENGINE=InnoDB")
                            // notifications
                        ];
                    case 15:
                        // feeds
                        _a.sent();
                        // notifications
                        return [4 /*yield*/, tablePool.query("CREATE TABLE `notifications` (`email` varchar(100) NOT NULL,  `notifyType` varchar(30) NOT NULL, `msgType` varchar(30) NOT NULL, `text` text NOT NULL, `date` datetime NOT NULL, PRIMARY KEY (`email`,`msgType`,`notifyType`))")];
                    case 16:
                        // notifications
                        _a.sent();
                    ...
                    case 17:
                        ...
                        _a.label = 18;
                    case 18: return [4 /*yield*/, auxPool.end()];
                    case 19:
                        _a.sent();
