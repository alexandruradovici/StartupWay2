...
export class UploadDownloadServer {
		...
		try {
			...
			if (conn) {
				const queryOptions: QueryOptions = {
					sql:
						"SELECT uploadDownload.* FROM uploadDownload where uploadDownload.productId=:productId"
				};
				let links = await conn.query(queryOptions, {
					productId: "7051998"
				});
				if (links.length > 0) {
					for (const link of links) {
						if (link.uuid !== "") {
							if (
								UploadDownloadServer.zips[link.uuid] ===
								undefined
							) {
	...
	): Promise<UploadDownloadLink | null> {
		let conn: PoolConnection | null = null;
		try {
			conn = await getPool().getConnection();
			if (conn) {
				...
				const response: UploadDownloadLink[] = await conn.query(
					queryOptions,
					{ uuid: uploadDownloadLink.uuid }
				);
	...
	async getLinksByProductIdAndFileType(
		productId: string,
		fileType: string
	): Promise<UploadDownloadLink[]> {
		let conn: PoolConnection | null = null;
		try {
			conn = await getPool().getConnection();
			if (conn) {
	...
	async getS3Object(uuid: string): Promise<string> {
		try {
			...
			if (uuid !== "" && uuid !== undefined) {
				...
				let utf8Data;
				const response: AWS.S3.GetObjectOutput = await s3
				...
				if (utf8Data !== undefined) return utf8Data;
				else return "";
			} else {
				return "";
			}
	...
	async getS3Url(uuid: string, userId?: string): Promise<string> {
		try {
			...
			if (userId !== undefined && userId !== "") {
				...
				if (user) name = user.username + "_profile_image.jpg";
			} else {
			...
			if (uuid !== undefined && uuid !== "" && process.env.BUCKET) {
				...
				if (
					s3.deleteObject(
						...
						(
							err: AWS.AWSError,
							data: AWS.S3.DeleteObjectOutput
						) => {
							if (err) {
		...
		try {
			...
			if (type === "all") {
				...
				for (const product of products) {
					...
					const usersArr = await teams.getUsersByTeamId(
						product.teamId
					);
				...
				if (city) {
					...
					for (const product of products) {
						...
						const users = await teams.getUsersByTeamId(
							product.teamId
						);
						const d = await teams.isTeamInDate(date, prId);
						if (users.length !== 0 && d) {
					...
					let links = await uploadDownload.getLinksByProductId(
						team,
						date
					);
					...
					if (links.length !== 0 && zip) {
						...
						for (const link of links) {
							const product = await teams.getProductById(
								link.productId
							);
							if (product && teamP) {
								const date = uploadDownload.formatDate(
					...
					for (const teamP of teamsArr) {
						...
						if (option !== undefined)
							links = await uploadDownload.getLinksByProductIdAndFileType(
								prId,
								option
							);
						else
							links = await uploadDownload.getLinksByProductId(
								prId,
								"none"
							);
						if (links.length !== 0) {
		...
		try {
			...
			if (type === "all") {
				...
				if (city) {
					...
					if (product) uuid = product.startupName + "_uploads_arhive";
					link = await uploadDownload.getLinkByUuid(uuid);
				} else if (
					typeof team === "object" &&
					typeof team[0] === "string"
					...
					else uuid = "demoday_uploads_arhive";
					link = await uploadDownload.getLinkByUuid(uuid);
				}
			}
			if (link) {
				...
				if (link.uuid === "" || newDate - oldDate >= 86400000) {
					if (UploadDownloadServer.zips[uuid] === null) {
						await uploadDownload.generateZip(
							type,
							date,
							uuid,
							option,
		...
		try {
			...
			if (link) {
				...
				if (file === ref) {
					...
					const url: string = await uploadDownload.getS3Url(
						link.uuid
					);
					if (url !== "") return url;
					else return "ERROR";
...
router.get(
	"/get/file/product/:fileType/:productId",
	async (
		...
		try {
			...
			if (
				...
				let links = await uploadDownload.getLinksByProductIdAndFileType(
					productId,
					type
				);
				const results = [];
				for (const link of links as UploadDownloadLink[]) {
					if ((link as UploadDownloadLink).uuid !== "") {
						const url = await uploadDownload.getS3Url(
							(link as UploadDownloadLink).uuid
						);
						results.push({
							data: url,
...
router.post(
	"/delete/file/",
	async (req: ApiRequest<{ uuid: string }>, res: ApiResponse<boolean>) => {
		try {
			...
			if (uuid !== "") {
				...
				} else res.status(500).send({ err: 500, data: false });
			}
		} catch (e) {
			console.error(e);
			res.status(500).send({ err: 500, data: false });
...
router.get(
	...
	async (req: ApiRequest<undefined>, res: ApiResponse<string | null>) => {
		try {
			...
			let url: string = "";
			if (uuid !== "") url = await uploadDownload.getS3Url(uuid);
			if (url !== "") {
		...
		try {
			...
			if (links.length !== 0) {
				...
				zip.generateNodeStream({
					...
					.pipe(
						fs.createWriteStream(path.join("/tmp", uuid + ".zip"))
					)
					.on("finish", async () => {
						console.log("Finished writing zip");
						...
						if (newLink) {
							...
							if (upload) {
								let url: string = "";
								url = await uploadDownload.getS3Url(
									newLink.uuid
								);
								if (url !== "") {
									await fs.remove(tmpFile);
									...
									await uploadDownload.deleteLink(
										newLink.uuid
									);
									await fs.remove(tmpFile);
...
router.post(
	...
	async (
		...
		try {
			...
			const team = req.body.team;
			const option: string = req.body.option;
			const response = await uploadDownload.getZip(
				type,
				date,
				option,
				city,
	...
	async (
		...
		try {
			...
			if (type === "team") {
				...
				if (product) uuid = product.startupName + "_uploads_arhive";
				let link;
				if (uuid) link = await uploadDownload.getLinkByUuid(uuid);
				let oldDate: number = 0;
				if (link) oldDate = new Date(link.uploadTime).getTime();
				const newDate = new Date().getTime();
				if (link && newDate - oldDate <= 86400000) {
					...
					let users;
					if (team) users = await teams.getUsersByTeamId(team.teamId);
				...
				if (link && newDate - oldDate <= 86400000) {
					...
					for (const product of products) {
						...
						const folder =
							product.location + "/" + product.teamName;
						if (users.length !== 0) {
							for (const user of users) {
								if (user)
									if (
			...
			if (!exists) {
						...
						.on("finish", async () => {
							...
							if (newLink) {
								...
								if (upload) {
									...
									await fs.remove(tmpFile);
									res.status(404).send(
										"Didn't upload ADDS3File"
									);
								}
							} else {
								res.status(404).send(
									"Didn't create link ADDLINK"
								);
							}
						});
				else {
				...
				if (newLink) url = await uploadDownload.getS3Url(newLink.uuid);

				if (url !== "") {
					res.status(200).send(url);
				} else {
					if (newLink) await uploadDownload.deleteLink(newLink.uuid);
					res.status(404).send("No url GETS3URL");
				}
			}
		} catch (e) {
			...
			if (!end) {
				...
				if (fileType !== "pres") {
					...
					try {
						...
						await ffmpeg(filePath).ffprobe(async function(
							err: any,
							metadata: any
						) {
							...
							if (
								...
								if (
									...
									let links: UploadDownloadLink[] = await uploadDownload.getLinksByProductIdAndFileType(
										productId,
										fileType
									);
									if (links.length > 0) {
										const file = await uploadDownload.getS3Object(
											links[0].uuid
										);
										if (file !== "") {
											if (
												await uploadDownload.deleteS3File(
													links[0].uuid
												)
											) {
												if (
													...
													await uploadDownload.addS3File(
														links[0].uuid,
														tmpFile,
														"path"
													);
								...
								const newLink = await uploadDownload.addLink(
									link
								);
								if (newLink) {
									const upload = await uploadDownload.addS3File(
					...
					if (links.length > 0) {
						...
						if (file !== "") {
							if (
								await uploadDownload.deleteS3File(links[0].uuid)
							) {
								if (
									!(await uploadDownload.deleteLink(
										links[0].uuid
...
router.post(
	...
	async (
		req: ApiRequest<{ base64Encode: string; userId: string }>,
		res: ApiResponse<boolean>
	) => {
		try {
	...
	async (
		...
		try {
			...
			if (userId !== "" && userId !== undefined) {
				const user = await users.getUserById(userId);
				let uuid;
				if (user) uuid = user.avatarUu;
				if (uuid !== "" && uuid !== null && uuid !== undefined) {
					const string = await uploadDownload.getS3Url(uuid, userId);
					if (string !== "") {
						res.status(200).send(string);
					} else res.status(500).send({ err: 500, data: null });
