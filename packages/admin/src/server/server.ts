...
import {
	getAuthorizationFunction,
	User,
	UsersServer
} from "@startupway/users/lib/server";
...
function parseEnum<D, T extends Record<string, D>>(
	t: T,
	key: string
): D | undefined {
...
export class AdminServer {
	 ...
	 * @param instagram - user instagram link
	...
	public parseCSVData(
		...
		try {
			...
			if (businessTrack !== undefined) {
				bT = (businessTrack as string)
					.toUpperCase()
					.replace(/\s+/g, "");
				if (bT === "Health&Lifestyle") bT = "HL";
			}
			let tT = "";
			...
			const bTValue = parseEnum<BusinessTrack, typeof BusinessTrack>(
				BusinessTrack,
				bT
			);
			const tTValue = parseEnum<TeamType, typeof TeamType>(TeamType, tT);
			const wDValue = parseEnum<WorkshopDay, typeof WorkshopDay>(
				WorkshopDay,
				wD
			);
			let btVal = "";
			...
			console.error(
				'Error in function "_parseCSVData(...array of params)"|"admin" '
			);
			console.error(error);
			return null;
		}
	...
	formatDate(date: Date): string {
		try {
			...
			else return new Date(date).toDateString() + " " + time[0];
		} catch (error) {
		...
		try {
			...
			if (conn) {
				const queryOptions: QueryOptions = {
					namedPlaceholders: true,
					sql:
						"SELECT t.location as 'oras', t.teamName as 'nume_echipa', p.businessTrack as 'business_track', p.teamType as 'type', p.descriptionRO as 'descriere_RO', p.descriptionEN as 'descriere_ENG' from teams t inner join products p on p.productId = t.productId and JSON_EXTRACT(productDetails,'$.assessmentSemifinals') = true;"
				};
				const response: any[] = await conn.query(queryOptions);
				if (response && response.length > 0) {
					return response;
	...
	async addRecovery(recovery: Recovery): Promise<Recovery | null> {
		...
		try {
			...
			if (conn) {
				...
				if (user) recovery.userId = user.userId;
				else throw new Error("No such user");

				const queryOptions: QueryOptions = {
					namedPlaceholders: true,
					sql:
						"INSERT INTO recoveries (recoveryId,userId,email,recoveryLink) VALUES(:recoveryId,:userId,:email,:recoveryLink)"
				};
				await conn.query(queryOptions, recovery);
				queryOptions.sql =
					"SELECT recoveryId,userId,email,recoveryLink FROM recoveries WHERE recoveryId=:recoveryId";
				const newRecovery: Recovery[] = await conn.query(queryOptions, {
				...
				if (newRecovery && newRecovery.length > 0 && newRecovery[0]) {
					const msg: string =
						...
						"		https://2022.teams.innovationlabs.ro/#/recovery/" +
						newRecovery[0].recoveryLink +
						"\n" +
					...
					const newNotification: SWNotify | null = await daemon.addNotification(
						notification
					);
					if (newNotification) {
						await conn.commit();
	...
	async deleteRecovery(recoveryId: string): Promise<boolean> {
		...
		try {
			...
			if (conn) {
				...
				queryOptions.sql =
					"SELECT recoveryId FROM recoveries WHERE recoveryId=:recoveryId";
				const response: {
					deleted_id: string;
				}[] = await conn.query(queryOptions, { recoveryId });
				if (response && response.length === 0) {
			...
			console.log(
				'Error in function "deleteRecovery(recoveryId)"|"admin"'
			);
			console.error(error);
			if (conn) {
				await conn.rollback();
	...
	async findRecoveryById(recoveryId: string): Promise<Recovery | null> {
		...
		try {
			...
			if (conn) {
				...
				const recovery: Recovery[] = await conn.query(queryOptions, {
					recoveryId
				});
		...
		try {
			...
			if (conn) {
				const queryOptions: QueryOptions = {
					namedPlaceholders: true,
					sql:
						"SELECT * FROM recoveries WHERE recoveryLink=:recoveryLink"
				};
				const recovery: Recovery[] = await conn.query(queryOptions, {
					recoveryLink
				});
			...
			console.log(
				'Error in function "findRecoveryByToken(recoveryLink)"|"admin"'
			);
			console.error(error);
			return null;
		} finally {
...
router.post(
	"/createResetEmail",
	async (
		req: ApiRequest<{ email: string }>,
		res: ApiResponse<Recovery | null>
	) => {
		try {
			...
			console.error(
				'Error on route "/createResetEmail" in "admin" router'
			);
			console.error(error);
...
router.post(
	"/resetPassword",
	async (
		req: ApiRequest<{ token: string; password: string }>,
		res: ApiResponse<{ username: string } | null>
	) => {
		...
		try {
			const recovery: Recovery | null = await admin.findRecoveryByToken(
				token
			);
			if (recovery) {
				const user: User | null = await users.getUserByEmail(
					recovery.email
				);
				if (user) {
...
router.post(
	"/checkToken",
	async (
		req: ApiRequest<{ token: string }>,
		res: ApiResponse<{ matched: boolean } | null>
	) => {
...
router.post(
	"/deleteRecovery",
	async (req: ApiRequest<{ token: string }>, res: ApiResponse<boolean>) => {
		/** @type {string} unique token */
		const token = req.body.token;
		try {
			const recovery: Recovery | null = await admin.findRecoveryByToken(
...
if (authFunct) router.use(authFunct);

router.post(
	"/updateDescriptionCSV",
	async (req: ApiRequest<{ encode: string }>, res: ApiResponse<boolean>) => {
		try {
			...
			for (const arr of parsed) {
				if (arr) {
					...
					if (object !== null) {
						const team = await teams.getTeamByYearAndLocation(
							new Date().getFullYear(),
							object.location,
							object.teamName
						);
						if (team) {
							const product = await teams.getProductByTeamId(
								team.teamId
							);
							if (product) {
								if (object.descEN !== "")
								...
								console.error(
									'"/updateDescriptionCSV" : Fetch Product Failed'
								);
							}
						} else {
							console.error(
								'"/updateDescriptionCSV" : Fetch Team Failed'
							);
						}
					} else {
						console.error(
							'"/updateDescriptionCSV" : Parse Function Failed'
						);
					}
				} else {
					console.error(
						'"/updateDescriptionCSV" : Papa Parse Failed'
					);
				}
			...
			console.error(
				'Error on route "/updateDescriptionCSV" in "admin" router'
			);
			console.error(error);
			res.status(401).send({ err: 401, data: false });
		}
 ...
 */ router.post(
	"/uploadCSV",
	async (req: ApiRequest<{ encode: string }>, res: ApiResponse<any>) => {
		try {
			/** @type {string} base64 string of the data found in the .csv */
			const encoded = req.body.encode;
			...
			for (const arr of parsed) {
				if (arr) {
					...
					if (object !== null) obj.push(object);
				}
				// WORKAROUND for parsing the csv data. TODO -> Create interface for parsing
			}
				...
				for (const entry of newObj[key]) {
					...
					if (entry.team && entry.product) {
						const mentorEmail: string =
							entry.team.teamDetails["mentor"];
						const mentor: User | null = await users.getUserByEmail(
							mentorEmail
						);
						let mentorUsername = "";
						...
						if (mentor) {
							...
							if (user) {
								const msg: string =
									...
									password +
									"\n" +
									"Use these credidentials to login on " +
									process.env.WEBHOSTNAME +
									"\n\n" +
									"Regards, Innovation Labs Team\n";
								const notification: SWNotify = {
									email: user.email,
						...
						team = await teams.getTeamByYearAndLocation(
							entry.team.year,
							entry.team.location,
							entry.team.teamName
						);
						if (team === null) {
							team = await teams.addTeam(
								entry.team,
								entry.product
							);
						}
					...
					if (user && team)
						userTeam = await teams.getUserInTeam(
							user.userId,
							team.teamId
						);
					if (userTeam === null && user !== null && team !== null) {
						...
						if (user && team && role !== undefined && role !== null)
							teamUser = await teams.addUserToTeam(
								user,
								team,
								role
							);
						...
						for (let i = 0; i < 10; i++) {
							...
							if (user !== null && teamUser !== null) {
								...
								const response = await teams.addActivityForUser(
									userActivity as UserActivity
								);
								if (!response) {
									console.error(
										'Error on route "/uploadCSV" in "admin" router'
									);
									console.error(
										"No activity added NO RESPONSE"
									);
									break;
								}
							} else {
...
router.post(
	"/newUserActivity",
	async (req: ApiRequest<UserActivity[]>, res: ApiResponse<boolean>) => {
		try {
			...
			if (userActivities) {
				for (const activity of userActivities) {
					const response: UserActivity | null = await teams.addActivityForUser(
						activity as UserActivity
					);
					if (!response) {
						console.error(
							'Error on route "/newUserActivity" in "admin" router'
						);
...
router.post(
	"/download/udc/data",
	async (req: ApiRequest<undefined>, res: ApiResponse<string | null>) => {
		try {
			...
			if (csv) {
				...
				console.error(
					'Error on route "/download/udc/data" in "admin" router'
				);
				console.error("No csv unparsed!");
				res.status(401).send({ err: 401, data: null });
			}
	...
	async (req: ApiRequest<undefined>, res: ApiResponse<string | null>) => {
		try {
			...
			if (csv) {
				...
				console.error(
					'Error on route "/download/team/data" in "admin" router'
				);
				console.error("No csv unparsed!");
				res.status(401).send({ err: 401, data: null });
			}
...
router.get(
	...
	async (
		...
		res: ApiResponse<(User & UserTeams)[]>
	) => {
		...
		try {
			if (req.params.location === "all") {
				...
				teamsArray = await teams.getTeamsByLocation(
					req.params.location
				);
			}
			...
			for (const team of teamsArray) {
				const auxUsers: (User &
					UserTeams)[] = await teams.getUsersByTeamId(team.teamId);
				users.push(...auxUsers);
			}
		...
		try {
			...
			if (users) {
				for (let user of users) {
					user.socialMedia = JSON.parse(
						(user.socialMedia as any) as string
					);
					user.userDetails = JSON.parse(
						(user.userDetails as any) as string
					);
				}
				res.send(users);
			} else {
	...
	async (
		...
		try {
			...
			for (let team of teamsArray) {
				team.productDetails = JSON.parse(
					(team.productDetails as any) as string
				);
				team.teamDetails = JSON.parse(
					(team.teamDetails as any) as string
				);
			}
			if (teamsArray) {
				res.send(teamsArray);
...
router.get(
	"/teams",
	async (req: ApiRequest<undefined>, res: ApiResponse<Team[]>) => {
		try {
...
router.post(
	"/teams/review",
	async (
		req: ApiRequest<{ type: string; location: string; id: string }>,
		res: ApiResponse<Review[]>
	) => {
		...
		try {
			...
			if (type === "admin")
				...
				teamsArray = await teams.getTeamAndProductByMentorId(
					req.body.id
				);
			...
			for (const team of teamsArray) {
				...
				const mentor: User | null = await users.getUserByEmail(
					JSON.parse(team.teamDetails as any)["mentor"]
				);
				...
				if (mentor && product) {
					...
					review = {
						...
						lastMentorUpdate: admin.formatDate(
							team.lastMentorUpdate
						)
					};
					reviews.push(review);
				}
			...
			if (reviews) {
				...
				console.error(
					'Error on route "/teams/review" in "admin" router'
				);
...
router.post(
	"/teams/review/update",
	async (
		req: ApiRequest<{ reviews: Review[]; type: string }>,
		res: ApiResponse<Review[]>
	) => {
		...
		try {
			for (const review of reviews) {
				const product: Product | null = await teams.getProductByTeamId(
					review.teamId
				);
				const team: Team | null = await teams.getTeamById(
					review.teamId
				);
				if (product && team) {
					if (type && type === "mentor") {
						product.productDetails["mentorNotes"] =
							review.mentorNotes;
					} else if (type && type === "admin") {
						product.productDetails["adminNotes"] =
							review.adminNotes;
					}
					...
					if (!newTeam) {
						console.error(
							'Error on route "/teams/review/update" in "admin" router'
						);
					...
					product.productDetails = JSON.parse(
						(product.productDetails as any) as string
					);
					product.productDetails["website"] = review.webLink;
					product.productDetails["assessmentSemifinals"] =
						review.assessmentSemifinals;
					product.productDetails["assessmentFinals"] =
						review.assessmentFinals;
					product.lastMentorUpdate = new Date(
						review.lastMentorUpdate
					);
					product.updatedAt = new Date(review.updatedAt);
					const prodRes: Product | null = await teams.updateProduct(
						product
					);
					if (prodRes) {
						revRes.push(review);
...
router.post(
	"/changeRole",
	async (
		req: ApiRequest<{ user: User; userTeam: UserTeams }>,
		res: ApiResponse<UserTeams | null>
	) => {
		...
		try {
			if (user) {
				...
				if (userTeam) await teams.updateUserTeamDetails(userTeam);
				else {
					console.error(
						'Error on route "/changeRole" in "admin" router'
					);
...
router.post(
	...
	async (
		...
		let mentor;
		if (product) mentor = await users.getUserById(product.mentorId);
		if (mentor && team) {
			const msg: string =
				"		From:" +
				from +
...
router.post(
	"/add/user",
	async (
		req: ApiRequest<{ user: User; option: string; teamId: string }>,
		res: ApiResponse<boolean>
	) => {
		try {
				...
				if (newUser) {
					if (option === "team") {
						...
						if (userTeam) {
							...
							if (team) {
								let initDate;
								const teamUser = await teams.addUserToTeam(
									newUser,
									team,
									role
								);
								if (teamUser) {
									...
									for (let i = 0; i < 10; i++) {
										...
										await teams.addActivityForUser(
											userActivity as UserActivity
										);
									}
								}
...
router.post(
	"/update/user",
	async (
		req: ApiRequest<{ user: User; changedPass: boolean }>,
		res: ApiResponse<boolean>
	) => {
		const user = req.body.user;
...
router.post(
	"/delete/user",
	async (req: ApiRequest<{ user: User }>, res: ApiResponse<boolean>) => {
		const user = req.body.user;
		if (user) {
			//TODO
			res.send(true);
...
router.post(
	"/request/user/team",
	async (
		req: ApiRequest<{ user: User; team: Team }>,
		res: ApiResponse<undefined>
	) => {
		const user = req.body.user;
		const team = req.body.team;
		if (user && team) {
			const userTeam = await teams.getUserInTeam(
				user.userId,
				team.teamId
			);
			if (userTeam) {
