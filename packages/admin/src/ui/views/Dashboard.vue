<template>
  <div>
    <v-container justify="center">
              ...
              <v-col class="d-flex align-end flex-column">
                <v-tooltip bottom>
                  <template v-slot:activator="{ on, attrs }">
                    ...
                    >
                      <v-icon color="secondaryDark1">mdi-email</v-icon>
                    </v-btn>
                  </template>
                ...
                <v-tooltip bottom>
                  <template v-slot:activator="{ on, attrs }">
                    <v-btn
                      v-on="on"
                      v-bind="attrs"
                      @click="changeRoute('/assessment')"
                      fab
                      outlined
                      right
                      color="secondaryDark1"
                    >
                      <v-icon color="secondaryDark1"
            ...
            >
                    ...
                    <v-card>
                      <v-form lazy-validation v-model="validDesc">
                        <v-card-title class="justify-center" style=""
                          >Approve Pending Description</v-card-title
                        >
                        <v-divider></v-divider>
                        <v-card-text>
                        ...
                        <v-card-actions>
                          <v-card-actions class="justify-center">
                            <v-btn
                              ...
                              >Approve Descriptions</v-btn
                            >
                            <v-btn
                              color="primary"
                              text
                              @click="exitApproveDialog()"
                              >Exit</v-btn
                            >
                          </v-card-actions>
                        </v-card-actions>
                  ...
                  >
                    <v-card outline>
                      <v-form lazy-validation v-model="valid" v-if="team">
                        <v-card-title class="justify-center" style=""
                          >Edit Team Details</v-card-title
                        >
                        <v-divider></v-divider>
                        ...
                        <v-card-actions class="justify-center">
                          <v-btn
                            ...
                            >Apply</v-btn
                          >
                          <v-btn color="primary" text @click="exitDialog()"
                            >Exit</v-btn
                          >
                        </v-card-actions>
                      </v-form>
              ...
              <template v-slot:[`item.updated`]="{ item }">
                <v-icon
                  ...
                  :disabled="disabledIcon(item)"
                  @click="openForApprove(item)"
                >
                  ...
                  <v-card flat outlined class="pa-0">
                    <v-list-item three-line>
                      ...
                      <v-list-item-avatar
                        ...
                        color="secondary"
                      >
                        <v-img :src="item.logo" icon> </v-img>
                      </v-list-item-avatar>
                      <v-list-item-avatar
                        v-else
                        tile
      ...
      <v-dialog persistent v-model="emailDialog" max-width="600px">
        <v-card>
          <v-form lazy-validation v-model="validEmail">
            <v-card-title class="justify-center" style=""
              >Send an email to teams</v-card-title
            >
            <v-divider></v-divider>
            <v-card-text>
              <div>Email Title</div>
              ...
              <v-select
                ...
                color="primary"
                :items="locations"
                v-model="email.location"
              ></v-select>
              <h5>
                If no checkbox is selected, the email will be sent to all teams
              </h5>
              <div>Teams that passed in to the semifinals</div>
              <v-simple-checkbox
                v-model="email.assessmentSemifinals"
              ></v-simple-checkbox>
              <div>Teams that passed in to the finals</div>
              <v-simple-checkbox
                v-model="email.assessmentFinals"
              ></v-simple-checkbox>
            </v-card-text>
            <v-card-actions>
              <v-card-actions class="justify-center">
                <v-btn
                  ...
                  >Send Emails</v-btn
                >
                <v-btn color="accent" text @click="emailDialog = false"
                  >Cancel</v-btn
                >
              </v-card-actions>
            </v-card-actions>
    ...
    >
      <v-list>
        <v-list-item>
          ...
          <v-list-item-title> Table Filters </v-list-item-title>
        </v-list-item>
      </v-list>
      ...
      <v-list nav dense>
        <v-list-item link>
          <v-list-item-icon>
            <v-icon v-if="teamTypeFilter.trim() !== ''" color="primary"
              >mdi-radar</v-icon
            >
          ...
          <v-list-item-content>
            ...
            >
              <template v-slot:prepend>
                <v-btn
                  v-if="teamTypeFilter.trim() !== ''"
                  icon
                  @click="teamTypeFilter = ''"
        ...
        <v-list-item link>
          <v-list-item-icon>
            <v-icon v-if="businessTracksFilter.trim() !== ''" color="primary"
              >mdi-domain</v-icon
            >
          ...
          <v-list-item-content>
            <v-select
              v-model="businessTracksFilter"
              :items="businessTracks"
              label="Business Track"
            >
              <template v-slot:prepend>
                <v-btn
                  v-if="businessTracksFilter.trim() !== ''"
                  icon
                  @click="businessTracksFilter = ''"
                  ><v-icon>mdi-close</v-icon></v-btn
                >
              </template>
            </v-select>
        ...
        <v-list-item link>
          <v-list-item-icon>
            <v-icon v-if="finalsFilter !== null" color="primary"
        ...
        <v-list-item link>
          <v-list-item-icon>
            <v-icon v-if="semifinalsFilter !== null" color="primary"
              >mdi-flag</v-icon
            >
        ...
        <v-list-item link>
          <v-list-item-icon>
            <v-icon v-if="locationFilter.trim() !== ''" color="primary"
              >mdi-city</v-icon
            >
        ...
        <v-list-item link>
          <v-list-item-icon>
            <v-icon v-if="workshopFilter.trim() !== ''" color="primary"
          ...
          <v-list-item-content>
            ...
            >
              <template v-slot:prepend>
                <v-btn
                  v-if="workshopFilter.trim() !== ''"
                  icon
                  @click="workshopFilter = ''"
                  ><v-icon>mdi-close</v-icon></v-btn
                >
              </template>
            </v-select>
...
interface IDashboard {
  ...
  rulesEmail: ((value: string) => string | boolean)[];
  rulesEmailTitle: ((value: string) => string | boolean)[];
  drawer: boolean;
  mini: boolean;
  valid: boolean;
  validDesc: boolean;
...
export default Vue.extend({
    ...
    return {
      ...
      rulesDesc: [
        (value: string) =>
          !value ||
          value.length <= 600 ||
          "Description must have at most 600 characters",
      ],
      rulesEmail: [
        (value: string) =>
          !value ||
          value.length <= 600 ||
          "Email must have at most 600 characters",
      ],
      rulesEmailTitle: [
        (value: string) => {
          if (value && value.length > 0) return true;
          else return "Email needs a title";
        },
      ],
      drawer: true,
      mini: true,
      ...
      teamTypes: ["Spin-off", "Start-up", "Scale-up"],
      businessTracks: [
        "Agriculture",
        "CyberSecurity",
        "FinTech",
      ...
      workshopDays: [],
      locations: ["Bucharest", "Cluj", "Iasi", "Sibiu", "Timisoara"],
      reviews: [] as Review[],
      type: "",
      loadingPage: false,
      loading: false,
      ...
      headers: [
        ...
        {
          text: "Last Mentor Update",
          value: "lastMentorUpdate",
          sortable: false,
        },
      ],
      tableOptions: {},
      ...
      email: {
        ...
        text: "",
        location: "",
        assessmentSemifinals: false,
        assessmentFinals: false,
      },
    };
  },
  filters: {
  ...
  watch: {
    ...
    selectedMentoredTeam: {
      ...
      async handler(newTeam: Team & Product): Promise<void> {
        if (newTeam) {
          try {
            let response = await this.ui.api.get<Product | null>(
              "/api/v1/teams/product/" + newTeam.teamId
            );
    ...
    async $route(to, from): Promise<void> {
      ...
      if (this.role === "Mentor") {
        let response = await this.ui.api.get<(Team & Product)[]>(
          "/api/v1/teams/mentor/teamsAndProduct/" + this.user.userId
        );
        if (response) {
        ...
        let response = await this.ui.api.get<(Team & Product)[]>(
          "/api/v1/admin/teams/" + this.user.userDetails["location"]
        );
        if (response) {
          this.mentoredTeams = this.modifyTeams(response.data);
        }
        ...
        if (newUser) {
          ...
          if (
            ...
            if (newUser.role === "Admin" || newUser.role === "SuperAdmin") {
              ...
              let response = await this.ui.api.get<(Team & Product)[]>(
                "/api/v1/admin/teams/" + newUser.userDetails["location"]
              );
              if (response) {
                this.mentoredTeams = this.modifyTeams(response.data);
                for (let team in this.mentoredTeams) {
                  this.mentoredTeams[team].teamId;
                  this.mentoredTeams[team].description =
                    response.data[team].descriptionEN;
                  this.mentoredTeams[team].mentor =
                    this.mentoredTeams[team].teamDetails.mentor;
              ...
              this.role = newUser.role;
              let response = await this.ui.api.get<(Team & Product)[]>(
                "/api/v1/teams/mentor/teamsAndProduct/" + newUser.userId
              );
              if (response) {
                this.mentoredTeams = this.modifyTeams(response.data);
                for (let team in this.mentoredTeams) {
          ...
          try {
            ...
            let response = await this.ui.api.post<Review[]>(
              "/api/v1/admin/teams/review",
              {
                type: this.type,
                id: id,
                location: location,
      ...
      async handler(newTeams: (Team & Product)[]): Promise<void> {
        if (this.role === "Mentor") {
          for (let team of newTeams) {
            let response = await this.ui.api.get<Product>(
              "/api/v1/teams/product/" + team.teamId
            );
  ...
  computed: {
    ...
    semifinalsFilter: {
      ...
      set(value: boolean | null): void {
        this.ui.storeDispatch("admin/updateSemifinalsFilter", value, {
          root: true,
        });
      },
    },
    finalsFilter: {
      ...
      set(value: boolean | null): void {
        this.ui.storeDispatch("admin/updateFinalsFilter", value, {
          root: true,
        });
      },
    },
    businessTracksFilter: {
      ...
      set(value: string): void {
        this.ui.storeDispatch("admin/updateBusinessTracksFilter", value, {
          root: true,
        });
      },
    },
    teamTypeFilter: {
      ...
      set(value: string): void {
        this.ui.storeDispatch("admin/updateTeamTypeFilter", value, {
          root: true,
        });
      },
    },
    locationFilter: {
      ...
      set(value: string): void {
        this.ui.storeDispatch("admin/updateLocationFilter", value, {
          root: true,
        });
      },
    },
    workshopFilter: {
      ...
      set(value: string): void {
        this.ui.storeDispatch("admin/updateWorkshopFilter", value, {
          root: true,
        });
      },
    },
        ...
        filteredRev = this.reviews.filter((review: Review) => {
          if (this.finalsFilter !== null) {
            if (this.semifinalsFilter !== null) {
              return (
                review.teamTrack.includes(this.teamTypeFilter) &&
                ...
                review.assessmentSemifinals === this.semifinalsFilter &&
                review.assessmentFinals === this.finalsFilter
              );
            }
            return (
              review.teamTrack.includes(this.teamTypeFilter) &&
  ...
  methods: {
    ...
    openForApprove(item: Team & Product): void {
      let toApprove = this.approveDescriptions.find(
        (el: Team & Product) => el.teamId === item.teamId
      );
      if (toApprove) {
    ...
    updateColor(item: Team & Product): string {
      let found = this.approveDescriptions.find(
        (element: Team & Product) => element.teamId === item.teamId
      );
      if (found) return "red";
      return "green";
    },
    ...
    _enumToData(enumData: any, name: string): void {
      ...
      for (let propName in enumData) {
        if (propName !== "NONE")
          ((this as any)[name] as Array<Object>).push(enumData[propName]);
      }
    },
    clearFilters(): void {
    ...
    async getLogo(event: { item: Review; open: boolean }): Promise<void> {
      try {
        if (!event.item.logo) {
          ...
          if (reviewIndex !== -1) {
            const resImage = await this.ui.api.get<
              { data: string; type: string; ext: string; uuid: string }[] | null
            >(
              `/api/v1/uploadDownload/get/file/product/logo/${event.item.productId}`
            );
    ...
    async goToTeam(item: Review) {
      ...
      if (this.$route.path !== path) this.$router.push(path + teamId);
    },
    async changeData() {
      this.valid = false;
      if (this.team !== null && this.team.startupName != "") {
        ...
        let productIndex = this.reviews.findIndex(
          (el: Review) =>
            el.startupName === (this.team as Team & Product).startupName
        );
        this.reviews[productIndex].updatedAt = (
          this.formatDate(new Date()) as unknown as Date
        ).toString();
        let response = await this.ui.api.post<Review[]>(
          "/api/v1/admin/teams/review/update",
          {
        ...
        if (response) {
          this.reviews = response.data;
          let productToUpdate = this.allTeams.findIndex(
            (el: Team & Product) => {
              return (
                el.startupName ===
                (this.reviews[productIndex] as Review).startupName
              );
            }
          );
          if (productToUpdate !== -1) {
            try {
              let product = await this.ui.api.get<Product | null>(
                "/api/v1/teams/product/" + this.allTeams[productToUpdate].teamId
              );
              if (product.data) {
                product.data.lastMentorUpdate = this.formatDate(
                  new Date()
                ) as unknown as Date;
                product.data.updatedAt = this.formatDate(
                  new Date()
                ) as unknown as Date;
                try {
                  await this.ui.api.post<Product | null>(
                    "/api/v1/teams/product/update",
                    {
              ...
              console.error(e);
              this.snackOptions.text =
                "Server Error. If the error persists, please contact technical support: teams@tech-lounge.ro.";
            ...
            let prd = {
              ...
              pendingDescriptionEN:
                this.allTeams[productToUpdate].pendingDescriptionEN,
              pendingDescriptionRO:
                this.allTeams[productToUpdate].pendingDescriptionRO,
    ...
    async approveDescription() {
      try {
        if (this.updated) {
          ...
          let response = await this.ui.api.post<Product | null>(
            "/api/v1/teams/product/approve/description",
            {
          ...
          if (response.data) {
            let res = await this.ui.api.get<Product | null>(
              "/api/v1/teams/product/" + this.selectedTeam
            );
            if (res) {
              ...
              if (this.updated) {
                ...
                this.updated.pendingDescriptionRO =
                  product.pendingDescriptionRO;
                this.updated.pendingDescriptionEN =
                ...
                let found2 = this.approveDescriptions.findIndex(
                  (el: Team & Product) =>
                    el.teamId === (this.updated as any).teamId
                );
                ...
                // as any because error that it's null
                let productIndex = this.reviews.findIndex(
                  (el: Review) => el.teamId === (this.updated as any).teamId
                );
                (this.reviews[productIndex] as Review).lastMentorUpdate = (
                  this.formatDate(new Date()) as unknown as Date
                ).toString();
              }
            }
    ...
    modifyTeams(newTeams: (Team & Product)[]): ModifiedTeam[] {
      ...
      for (let team of newTeams) {
        ...
        team.lastMentorUpdate = new Date(
          this.formatDate(team.lastMentorUpdate)
        );
        this.allTeams.push(team);
        if (
          team.pendingDescriptionEN !== "" ||
          team.pendingDescriptionRO !== ""
        ) {
    ...
    checkRoute(): boolean {
      if (this.$router.currentRoute.path === "/workspace") return true;
      else return false;
    },
    pushBack(): void {
      this.$router.go(-1);
    ...
    changeRoute(link: string): void {
      if (
        ...
        this.selectedMentoredTeam &&
        this.selectedMentoredTeam.teamId !== "" &&
        link.split("/")[1] === "viewTeam"
      ) {
    ...
    async sendEmail() {
      try {
        const emailSent = await this.ui.api.post<boolean>(
          "/api/v1/teams/email/",
          this.email
        );
        if (emailSent && emailSent.data) {
          this.snackOptions.text = "Emails are being sent!";
          this.snackOptions.type = SnackBarTypes.SUCCESS;
          this.snackOptions.timeout = 2000;
          this.snackbar = true;
        } else {
          this.snackOptions.text = "Emails were not sent!";
          this.snackOptions.type = SnackBarTypes.ERROR;
          this.snackOptions.timeout = 2000;
          this.snackbar = true;
        }
