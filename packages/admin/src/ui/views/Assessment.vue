					...
					<v-card color="primary" flat shaped outlined width="100%">
						<v-card flat shaped outlined width="100%" class="pa-2">
							...
							>
								...
								<template
									v-slot:[`item.assesSemiFinals`]="{ item }"
								>
									<v-simple-checkbox
										color="#197E81"
		...
		>
			...
			<v-list nav dense>
				<v-list-item link>
					<v-list-item-icon>
						<v-icon
							v-if="teamTypeFilterAssessment.trim() !== ''"
							color="#197E81"
							>mdi-radar</v-icon
						>
					...
					<v-list-item-content>
						<v-select
							v-model="teamTypeFilterAssessment"
							:items="teamTypes"
							label="Team Track"
						>
							<template v-slot:prepend>
								<v-btn
									v-if="
										teamTypeFilterAssessment.trim() !== ''
									"
									icon
									@click="teamTypeFilterAssessment = ''"
				...
				<v-list-item link>
					<v-list-item-icon>
						<v-icon
							v-if="businessTracksFilterAssessment.trim() !== ''"
							color="#197E81"
							>mdi-domain</v-icon
						>
					...
					<v-list-item-content>
						<v-select
							v-model="businessTracksFilterAssessment"
						...
						>
							<template v-slot:prepend>
								<v-btn
									v-if="
										businessTracksFilterAssessment.trim() !==
										''
									"
									icon
									@click="businessTracksFilterAssessment = ''"
									><v-icon>mdi-close</v-icon></v-btn
								>
							</template>
						</v-select>
				...
				<v-list-item link>
					<v-list-item-icon>
						<v-icon
							v-if="finalsFilterAssessment !== null"
							color="#197E81"
					...
					<v-list-item-content>
						<v-select
							v-model="finalsFilterAssessment"
							:items="values"
							item-text="text"
							item-value="value"
				...
				<v-list-item link>
					<v-list-item-icon>
						<v-icon
							v-if="semifinalsFilterAssessment !== null"
							color="#197E81"
							>mdi-flag</v-icon
						>
					...
					<v-list-item-content>
						<v-select
							v-model="semifinalsFilterAssessment"
							:items="values"
							item-text="text"
							item-value="value"
				...
				<v-list-item link>
					<v-list-item-icon>
						<v-icon
							v-if="locationFilterAssessment.trim() !== ''"
							color="#197E81"
							>mdi-city</v-icon
						>
					...
					<v-list-item-content>
						<v-select
							v-model="locationFilterAssessment"
							:items="locations"
							label="Location"
						>
							<template v-slot:prepend>
								<v-btn
									v-if="
										locationFilterAssessment.trim() !== ''
									"
									icon
									@click="locationFilterAssessment = ''"
									><v-icon>mdi-close</v-icon></v-btn
								>
							</template>
						</v-select>
...
export default Vue.extend({
	...
	data() {
		return {
			...
			teams: [] as (Team &
				Product & { assesFinals: boolean; assesSemiFinals: boolean })[]
		};
	},
	filters: {
				...
				if (newUser) {
					...
					if (
						...
						if (
							...
							>(
								"/api/v1/teams/mentor/teamsAndProduct/" +
									newUser.userId
							);
							if (response) {
								this.teams = this.modifyTeams(response.data);
	...
	computed: {
		...
		sortByAssessment: {
			...
			set(value: string | string[]): void {
				this.ui.storeDispatch("admin/updateAssessmentSortBy", value, {
					root: true
				});
			}
		},
		sortDescAssessment: {
			...
			set(value: string | string[]): void {
				this.ui.storeDispatch("admin/updateAssessmentSortDesc", value, {
					root: true
				});
			}
		},
		searchAssessment: {
			...
			set(value: string): void {
				this.ui.storeDispatch("admin/updateAssessmentSearch", value, {
					root: true
				});
			}
		},
		semifinalsFilterAssessment: {
			...
			set(value: boolean | null): void {
				this.ui.storeDispatch(
					"admin/updateAssessmentSemifinalsFilter",
					value,
					{
						root: true
					}
		...
		finalsFilterAssessment: {
			...
			set(value: boolean | null): void {
				this.ui.storeDispatch(
					"admin/updateAssessmentFinalsFilter",
					value,
					{
						root: true
					}
		...
		businessTracksFilterAssessment: {
			...
			set(value: string): void {
				this.ui.storeDispatch(
					"admin/updateAssessmentBusinessTracksFilter",
					value,
					{ root: true }
				);
			}
		...
		teamTypeFilterAssessment: {
			...
			set(value: string): void {
				this.ui.storeDispatch(
					"admin/updateAssessmentTeamTypeFilter",
					value,
					{
						root: true
					}
		...
		locationFilterAssessment: {
			...
			set(value: string): void {
				this.ui.storeDispatch(
					"admin/updateAssessmentLocationFilter",
					value,
					{
						root: true
					}
		...
		filteredTeams(): (Team &
			...
			if (this.teams.length > 0) {
				filteredTeam = this.teams.filter(
					(
						team: Team &
							Product & {
								assesFinals: boolean;
						...
						if (this.finalsFilterAssessment !== null) {
							if (this.semifinalsFilterAssessment !== null) {
								return (
									team.teamType.includes(
										this.teamTypeFilterAssessment
									) &&
									team.businessTrack.includes(
										this.businessTracksFilterAssessment
									) &&
									team.location.includes(
										this.locationFilterAssessment
									) &&
									team.assesFinals ===
										this.finalsFilterAssessment &&
									team.assesSemiFinals ===
										this.semifinalsFilterAssessment
								);
							}
							return (
								team.teamType.includes(
	...
	methods: {
		...
		_enumToData(enumData: any, name: string): void {
			...
			for (let propName in enumData) {
				if (propName !== "NONE")
					((this as any)[name] as Array<Object>).push(
						enumData[propName]
					);
			}
		},
		...
		modifyTeams(
			...
			for (let team of newTeams) {
				team.assesSemiFinals =
					team.productDetails["assessmentSemifinals"];
		...
		async updateProduct(
			...
			try {
				const response = await this.ui.api.get<Product | null>(
					"/api/v1/teams/product/" + item.teamId
				);
				const product = response.data;
				if (product) {
					product.productDetails.assessmentFinals = item.assesFinals;
					product.productDetails.assessmentSemifinals =
						item.assesSemiFinals;
					const resp: Product | null = await this.ui.api.post(
						"/api/v1/teams/product/update",
						{ teamId: item.teamId, product: product }
					);
					if (!resp) {
						console.error("ERROR");
					}
