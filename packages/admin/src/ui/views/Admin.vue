<template>
  <div>
    <v-navigation-drawer
      ...
      clipped
      permanent
      style="border-left: #ffb100 solid 10px"
    >
      <v-list>
...
export default Vue.extend({
      ...
      async handler(newUser: User): Promise<void> {
        if (newUser) {
          if (newUser.role === "Admin" || newUser.role === "SuperAdmin") {
            try {
              const response = await this.ui.api.get<(Team & Product)[]>(
                "/api/v1/teams/mentor/teams/" + newUser.userId
              );
              if (response) {
  ...
  methods: {
    ...
    async getAllUsers(): Promise<boolean> {
      try {
        const response = await this.ui.api.get<User[]>(
